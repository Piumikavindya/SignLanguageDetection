...
# Choose one:
model_type = 'mlp'  # 'mlp' or 'rf'

if model_type == 'mlp':
    model = load_model('../model_training/mlp_landmarks.h5')
else:
    model = joblib.load('../model_training/rf_landmark_model.joblib')

with open('../model_training/label_encoder.pkl', 'rb') as f:
    le = pickle.load(f)
...
def extract_landmarks_from_frame(frame):
    ...
    if not res.multi_hand_landmarks:
        return None
    paired = list(zip(res.multi_hand_landmarks, [h.classification[0].label for h in res.multi_handedness]))
...
while cap.isOpened():
    ...
    if landmarks is not None:
        ...
        cv2.putText(frame, pred_class, (10,40), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0,255,0), 2)
